...
export default class BrandRepository {
    ...
    async create(brand) {
        try {
            ...
            const result = await request.query("INSERT INTO brands (name, slug, status) VALUES (@name, @slug, @status); SELECT SCOPE_IDENTITY() AS id");
    ...
    async findAll() {
        try {
            const pool = await getPool();
            const request = pool.request();
            const result = await request.query("SELECT * FROM brands ORDER BY id DESC");
            return result.recordset.map((brand) => new Brand({
                id: brand.id,
                name: brand.name,
                slug: brand.slug,
                status: brand.status,
    ...
    async update(id, item) {
        try {
            ...
            request.input("id", sql.Int, id);
            const query = `UPDATE brands SET ${updateFields.join(", ")} WHERE id = @id`;
