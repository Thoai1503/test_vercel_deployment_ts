...
export default class UserRepository {
    ...
    async createUser(user) {
        ...
        const result = await request.query("INSERT INTO users (full_name, email, phone, password, role, status) VALUES (@name, @email, @phone, @password, @role, @status); SELECT SCOPE_IDENTITY() AS id");
    ...
    async updateUser(id, user) {
        ...
        request.input("role", sql.Int, user.getRole());
        request.input("status", sql.Int, user.getStatus());
        await request.query("UPDATE users SET name = @name, email = @email, phone = @phone, password = @password, role = @role, status = @status WHERE id = @id");
        return { id };
    }
}
